...
*/			
	...
	/**
	 * Get an array of all sale and regular prices from all variations. This is used for example when displaying 
	 * the price range at variable product level or seeing if the variable product is on sale.
	 *
	 * Can be filtered by plugins which modify costs, but otherwise will include the raw meta costs unlike get_price() 
	 * which runs costs through the woocommerce_get_price filter.
	...
	public function read_price_data( &$product, $for_display = false ) {
		...
		/**
		 * $this->prices_array is an array of values which may have been modified from what is stored in transients - 
		 * this may not match $transient_cached_prices_array.
		 * If the value has already been generated, we don't need to grab the values again so just return them. 
		 * They are already filtered.
		 */
		if ( empty( $this->prices_array[ $price_hash ] ) ) {
			 ...
			 * which has been generated and store locally to the class.
	...
	/**
	 * Create unique cache key based on the tax location (affects displayed/cached prices), 
	 * product version and active price filters.
...
*/
			...
			if ( 'yes' === get_option( 'woocommerce_hide_out_of_stock_items' ) ) {
				$dgc_API_args['query']['stock_status'] = 'instock';
			}
			$dgc_API_res = dgc_API_call('/retrieveRecords/', 'POST', $dgc_API_args);
			foreach(json_decode($dgc_API_res['body']) as $dgc_API_row) {
